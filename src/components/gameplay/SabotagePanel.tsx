...
const defaultActions: SabotageAction[] = [
  ...
  { id: "hint", name: "HINT", icon: <Lightbulb className="w-3.5 h-3.5" /> }, // O nosso alvo!
...
export function SabotagePanel({ actions = defaultActions, onActionClick }: SabotagePanelProps) {
  ...
  // 👉 A LÓGICA QUE FALTAVA: Intercepta o clique antes de passar pro seu layout
  const handleActionClick = (actionId: string) => {
    // Se for o botão de dica, "grita" pra engine do Phaser gerar a pista
    if (actionId === "hint") {
      window.dispatchEvent(new CustomEvent('sudocidio:requestHint'));
    }
    
    // Continua executando a função original caso a página precise no futuro
    if (onActionClick) {
    ...
    <div className="panel flex flex-col">
      ...
      <div className="p-2 grid grid-cols-2 gap-1.5">
        {actions.map((action) => (
          <button
            key={action.id}
            onClick={() => handleActionClick(action.id)} // 👉 Chama a nossa função nova
