...
export function OptionsMenu() {
  ...
  // Avisa o Phaser sempre que o volume ou o mudo mudarem
  useEffect(() => {
    const finalVolume = isMuted ? 0 : volume / 100;
    window.dispatchEvent(new CustomEvent('sudocidio:volumeChange', { detail: { volume: finalVolume } }));
  }, [volume, isMuted]);

  const handleGiveUp = () => {
    // Redireciona para o Menu Principal (ajuste o caminho se necessário)
    window.location.href = "/";
  };

  return (
    <>
      {/* Botão de Engrenagem (Fica visível na tela) */}
      <button 
      ...
      {/* Modal Overlay (Fundo escuro embaçado) */}
      {isOpen && (
